...
Steps:
1. Run `python -c "import pathlib; d=pathlib.Path('${CLAUDE_SKILL_DIR}/program_data'); d.mkdir(exist_ok=True); [f.unlink() for f in d.iterdir() if f.is_file()]"` to ensure the output directory exists and clear any previous session files.
...
4. Run this Bash command (foreground, timeout 35000) to poll for the Microsoft auth output:
   ...
   - If no output appears after 30 seconds, authentication was cached and already completed — continue to step 5.
